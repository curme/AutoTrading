...
class pnlCalculator:
        ...
        for s in self.strategy:
            ...
            for i in self.tradebook.index:
                if self.tradebook.loc[i,'Strategy'] == s:
                    if self.tradebook.loc[i,'Action']=="BuyToCover" or self.tradebook.loc[i,'Action']=="SellToCover":
    ...
    def run(self):
        ...
        for i in xrange(len(self.strategy)):
            performance.append([self.strategy[i], self.realizedpnl[i], self.actualreturn[i], self.annualizedreturn[i], self.volatility[i], self.Sharpratio[i], self.mdd[i]])
        report = pd.DataFrame(performance, columns=["Strategy" , "realized PnL", "Return", "Annualized Return", "Volatility", "Sharpe Ratio", "MDD"])
        print "Report " + "/" * 100
        print report